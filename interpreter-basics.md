...
**インタプリタ（interpreter）**とは、プログラムのソースコードを読み込み、その意味を解釈しながら実行するプログラムです。同じ「言語処理系」でも、ソースコードを機械語やバイトコードなどの形式に翻訳してファイルとして出力する**コンパイラ（compiler）**とは役割が違います。コンパイラが「翻訳して終わり」なのに対し、インタプリタは「翻訳しながら（あるいは翻訳せずに）その場で実行する」のが特徴です[Nystrom, 2021](#cite:nystrom2021)。
...
`1` や `2` のような数値は、メモリに直接そのまま置けます。ところが、文字列やリストのように「大きさが決まっていない、あるいは複数の部品からなるデータ」は、そうはいきません。こうしたデータはヒープに割り当てられ、プログラムはそれを**ポインタ**経由で扱います。

本書では、ヒープに割り当てられて GC の管理対象になるデータを**オブジェクト（object）**と呼びます。一方、数値のようにヒープを使わずその場に置ける値は、GC の管理外です。
...
- 文字列やリストのような大きなデータは**オブジェクト**としてヒープに割り当てられ、GC の管理対象になる。数値などの即値は管理対象外。
