...
たとえば `1 2 3 nil cons cons cons` というプログラムを実行すると、リスト `(1 2 3)` がスタックに残ります。`cons` と `str:` がオブジェクト（ペアと文字列）をヒープに割り当てるので、ここに GC が必要になります。リストや文字列を扱うこの設計は、GC 発祥の地である LISP のミニチュア版でもあります[McCarthy, 1960](#cite:mccarthy1960)。
