...
前章で、動くコピー GC が手に入りました。しかし「動く」と「速い」は別物です。素朴なコピー GC には、無視できない無駄があります。この章では、その無駄を取り除き、GC を実用的な速さにするための代表的なテクニックを、考え方を中心に紹介します。コードの完成度よりも「なぜそれが効くのか」を理解することを目指します。
...
この「オブジェクトの性質に応じて領域と回収方式を使い分ける」という発想は、世代別 GC の延長線上にあります。若い／古い、小さい／大きい、ポインタを多く含む／含まない。such な性質ごとに最適な扱いをするのが、実用的な GC の腕の見せどころです。
...
```mermaid
graph TD
    A["まず: 動くコピー GC<br>（本書の実践編）"] --> B["次に: 世代別化<br>新世代をコピー方式に"]
    B --> C["世代をまたぐ参照のために<br>ライトバリア＋記憶集合"]
    C --> D["大きなオブジェクトを別領域へ"]
    D --> E["必要なら: インクリメンタル GC で<br>停止時間を短縮"]
```
...
- 素朴なコピー GC は、**長命オブジェクトの再コピー**という無駄を抱える。
- **弱い世代別仮説**（多くのオブジェクトは若くして死ぬ）に基づき、若いオブジェクトを頻繁・集中的に回収するのが**世代別 GC**。新世代の回収にコピー方式が最適。
