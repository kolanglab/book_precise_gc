...
インタプリタとは、プログラムのソースコードを読み込み、その意味を解釈しながら実行するプログラムのことです[Nystrom, 2021](#cite:nystrom2021)。たとえば電卓に `1 + 2 * 3` と打ち込むと `7` が返ってくる、あの「式を理解して計算する部分」を一般化したものだと考えてください。
...
- **保守的 GC** は、ポインタかどうか確信が持てないまま、「ポインタかもしれない」ものを手がかりに動きます[Boehm and Weiser, 1988](#cite:boehm1988)。手軽ですが、できることに制限があります。
...
本書はあくまで「最初の一歩」を踏み出すための入門書です。GC は半世紀以上の歴史を持つ奥深い分野で、本書で触れられるのはそのごく一部です。もっと体系的に学びたくなったら、GC の理論と実装を網羅的にまとめた『The Garbage Collection Handbook』[Jones et al., 2023](#cite:jones2023) が定番の参考書になります。インタプリタ作りそのものをもっと知りたい場合は『Crafting Interpreters』[Nystrom, 2021](#cite:nystrom2021) が読みやすくおすすめです。
