...
なお、ここでの `chars` や `items` は、GC ヒープの**外**に `malloc` で確保したバッファを指す想定です。GC ヒープ外のメモリは GC の管理対象ではないので、オブジェクトが死んだときに誰がそのバッファを解放するか、という問題が別に残ります（コピー方式はごみを「コピーしないことで捨てる」ため、ごみに紐づく外部バッファの解放を差し挟む場所がないのです）。実践編では、文字列の本体をオブジェクト末尾に直接埋め込む方式に変えて、この問題ごと消します。
...
> [!NOTE]
> ハンドルスタックは手作業で書く必要があり、登録・解除を間違えやすい部分です。とくに関数の途中で早期 `return` したり、エラー処理へ `goto` で抜けたりする経路では、`pop_roots` を通らずに関数を出てしまう「解除漏れ」が起きがちです（解除漏れは、死んだはずのローカル変数の番地をルートとして使い続けるので、メモリ浪費どころか未定義動作になりえます）。実際の言語処理系では、スコープの入口でスタック位置を覚えて出口で必ず巻き戻すマクロや専用 API で半自動化したり、そもそもポインタを移動させないなどの工夫で負担を減らしたりします。本書ではしくみを理解するために、あえて手で書く方式を採ります。
